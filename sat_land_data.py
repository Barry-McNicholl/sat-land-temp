...
def main():
    # Previous year should have data for each day so, collect data from this
    # year and up to four years earlier
    last_year = datetime.now().year-1
    # Years to collect data from
    data_year = [last_year-4, last_year-3, last_year-2, last_year-1, last_year]
    ...
    for i in range(len(regions)): # For each region
        for j in range(len(data_year)): # For every year
            ...
            year = str(data_year[j])
            print('Getting Daily Land Temperatures in ' + regions[i]
                  + ' for ' + year)
            # Make the api call for land data
            land_request = requests.get('https://www.ncdc.noaa.gov/cdo-web/'
                                        + 'api/v2/data?datasetid=GHCND&'
            ...
            # Check in the response that the token was valid
            if 'message' in land_response:
